...
import { UpdateUserDto, UserWithStats, UsageStats, DailyUsage, ConversationUsage } from "./user.types";

// Prices from env with GPT-4o-mini defaults
const PRICE_INPUT_PER_1M  = parseFloat(process.env.PRICE_INPUT_PER_1M  ?? "0.15");
const PRICE_OUTPUT_PER_1M = parseFloat(process.env.PRICE_OUTPUT_PER_1M ?? "0.60");
const PRICE_WHISPER_PER_MIN = parseFloat(process.env.PRICE_WHISPER_PER_MIN ?? "0.006");
const PRICE_TTS_PER_1M_CHARS = parseFloat(process.env.PRICE_TTS_PER_1M_CHARS ?? "15.00");

function calcCost(inputTokens: number, outputTokens: number, audioSeconds: number, ttsChars: number): number {
  return (
...
function periodBounds(period: "day" | "week" | "month" | "all"): { from: Date; prevFrom: Date; prevTo: Date } {
  const now = new Date();
...
export const userService = {
  ...
  async getUsageStats(userId: string, period: "day" | "week" | "month" | "all"): Promise<UsageStats> {
    ...
    const totalCost = calcCost(totalInputTokens, totalOutputTokens, totalAudioSeconds, totalTtsChars);
    ...
    if (period !== "all") {
      const prevMessages = await prisma.message.findMany({
        ...
        select: { role: true, content: true, inputTokens: true, outputTokens: true, audioSeconds: true },
      });
      ...
      const prevCost = calcCost(
        ...
        prevMessages.filter((m) => m.role === "assistant").reduce((s, m) => s + m.content.length, 0),
      );

      costDelta     = prevCost     > 0 ? ((totalCost     - prevCost)     / prevCost)     * 100 : null;
      messagesDelta = prevMessages.length > 0 ? ((messages.length - prevMessages.length) / prevMessages.length) * 100 : null;
    }
    ...
    for (const m of messages) {
      ...
      if (!dailyMap.has(date)) {
        dailyMap.set(date, { date, inputTokens: 0, outputTokens: 0, audioSeconds: 0, cost: 0, messages: 0 });
      }
      ...
      day.messages     += 1;
      day.cost = calcCost(
        day.inputTokens, day.outputTokens, day.audioSeconds,
        messages.filter((x) => x.createdAt.toISOString().slice(0, 10) === date && x.role === "assistant")
    ...
    const topConversations: ConversationUsage[] = conversations
      .map((c) => {
        ...
        const tts  = c.messages.filter((m) => m.role === "assistant").reduce((s, m) => s + m.content.length, 0);
        return {
          id: c.id,
          title: c.title,
